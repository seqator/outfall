...
export interface WeaponRuntimeState {
  ...
  /**
   * OF-057 (P0-3 баланс-прохода, `docs/qa/balance-report.md`): сколько
   * патронов этого оружия реально доступно вне магазина — «жёсткий ресурс»
   * (`docs/design/combat.md` §1), а не бесконечная перезарядка по нажатию
   * `R`. `sim` не импортирует `game/inventory` (граница слоёв,
   * `docs/tech/architecture.md` §4) и потому не может сам посчитать, сколько
   * патронов лежит в вещмешке — это поле лишь зеркалит то число, которое
   * раз в кадр пишет сюда `demo-scene.ts` (единственный слой с одновременным
   * доступом к `World` и `InventoryState`, тот же принцип, что уже применяют
   * `applyItemUseEffects`/`applyArenaModifiersToInput`). `combatSystem`
   * только читает и уменьшает это число при перезарядке — реальное списание
   * из инвентаря происходит в `demo-scene.ts` по событию
   * `combat.reload-finish` (`sim/events.ts`). 0 у оружия без патронов
   * (Кулаки) — никогда не читается (перезарядка для него не запускается,
