...
export async function createDemoScene(
   ...
   * оружия, — `item.pistol_ogryzok`/`item.shotgun_duplo`) — данные, не
   * выдумка: если оружие когда-нибудь появится в `AMMO_WEAPON_IDS` без
   * записи в `ItemRegistry` или без единой точки подбора патрона на картах,
   * его резерв честно останется `0` навсегда после стартового магазина —
   * это не гипотетический случай, а найденная и исправленная девятой
   * рецензией duxa-simulator дыра (`item.shotgun_duplo` не был
   * зарегистрирован вообще, `item.ammo_shotgun`/пикапы добавлены следом,
   * `OF-060`). «Кран» (без магазина) сюда не входит — у него нет и не может
   * быть резерва.
   */
