 ...
 *
 * Геометрия `src/game/world/dev-fixtures.ts`: спавн (32,32) — это 2-тайловый
 * проход (x=31/32) в горизонтальной перегородке на y=32, которая иначе
 * перекрывает всю строку от x=16 до x=47. Из-за этого движение на восток
 * или запад из спавна почти сразу упирается в саму перегородку (~1 тайл) —
 * это НЕ дальняя стена периметра. Для проверки «долгого» упора в реальную
 * стену периметра уходим на север (открытая часть комнаты выше перегородки,
 * без препятствий на этой колонке) — там до стены y=0 остаётся ~32 тайла.
 */
